...
- **カスケード削除の推移的検出**: `mergeBranches` が base のグラフを受け取る形にする
  ...
  > **これは仕様に無い要求を計画が足している。**`merging.md` の S1/S2/S4 は直接参照だけを
  > 挙げており「推移的」を要求していない。スコープの膨張ではなく**仕様の穴を埋めている**
  > ケースなので、決着したら仕様側へ戻すこと
...
- **承認の述語を決める** (U8)。「全員が承認したか」を名簿の食い違いの下でどう定義するか。
  **これは仕様に戻る話**なので、Phase 3 の設計までに決着させておく
...
| **U8** | **⚠️ 承認と名簿の食い違いが両立しない (仕様の穴)。**「呼び出された actor **全員**が承認したら再 merge が可能」は**名簿を入力にする述語**である。ところが名簿の食い違いは許容すると決めた。したがって **a の端末では「全員承認済 → 再 merge 可能」、b の端末では「1 人足りない → 保留」が同時に、どちらも正常な状態として成立する**。再 merge は trunk を書き換えるので表示の食い違いでは済まない。DtR が保留中に承認者が参加を取りやめた場合の扱いも同じ問題に含まれる。**仕様に戻って決める必要がある** | **仕様の改訂が要る。Phase 3 の設計まで** |
...
| | 内容 |
| --- | --- |
| **S1** | **承認と名簿の食い違いが両立しない** (U8)。「呼び出された actor **全員**が承認したら再 merge」は名簿を入力にする述語だが、名簿の食い違いは許容すると決めた。**a の手元では「全員承認済」、b の手元では「1 人足りない」が同時に正常として成立する**。再 merge は trunk を書き換えるので表示の食い違いでは済まない。`participation.md` と `dialogueToResolveGraph.md` のどちらに手を入れるかを含めて決める |
| **S2** | **型チェックの時期が矛盾している。** `propertyEditor.md` は「型制約のチェックは今回は行わない (→ step 3)」を 2 箇所で書き、`template.md` は template 実行に「プロパティの値を設定するときの型チェック」を step2 として置いている。**本計画は propertyEditor 側を採った**が、仕様を直すなら `template.md` 側を step3 へ寄せるのが自然である。あわせて `propertyEditor.md` の「型がすでに決まっている場合の値を変更しようとした時」が step3 の記述であることを明示する |
| **S3** | **カスケード削除の推移的な検出が仕様に無い。** `merging.md` の S1/S2/S4 は直接参照だけを挙げている。計画は「削除された親の子孫」まで追うことにしたが、これは仕様の穴を埋めている。決着したら仕様へ戻す |

...
> **step2 は完了した (2026-09-23)。**Phase 0〜7 すべて実装済み。
> **振り返りと step3 の着手材料は [step3-entry](./step3-entry.md) にある** —
> 着手前に決めるべき 6 件、決めるべき問い 26 件、確信の度合いと見ていない範囲。
>
> **この文書は step2 の記録として閉じる。**以降の判断は step3-entry を入口にすること。
...
- **fork に競合の原因を凍結して付ける** (2026-08-31、PR #213 のコメントで追加)。fork には
  explicit merge のような紐づく merge 操作がないので、**検出の瞬間に書かないと理由を指す先が
  無くなる**。しかも畳み直しでは復元できない — fork の後も op-log は伸びるし、負けた側が
  その後に消えていれば競合そのものが再現しない。**種別 / 対象の id とその時点で人間に見える形 /
  双方の op と actor と clock / 分岐点**を凍結する。`MergeConflict` が `batchId` を持つので
  actor と clock は導けて、足すのは「人間に読める形」だけである。
  **これは記述であって畳み込みの入力ではない** — 唯一の用途は、この fork から DtR を
  起動するときの呼び出し対象の既定値を供給することである
...
| **Q3** | 実機で通す ✅ **12/12 PASS** (`src/client/data-p4/p4.ts`, 投棄前提) |

**設計文書は起こさない。**§4 の基準は「大きい Phase (2, 3, 6)」であり、Phase 4 は
差し込み枠なのでここに書く (Phase 7 と同じ扱い)。

#### ⚠️ 「型をどこから得るか」の決定を撤回した (2026-09-20)

上の「**決定: 値から推論して表示するだけにする**」は**誤りだったので撤回する**。
利用者の判断は次のとおりで、仕様 (`propertyEditor.md`) もこれに合わせて直した。

> **あるプロパティの型を指定するのは, 実装コードか, template のような拡張である。**
> システムや拡張で使われるプロパティならば想定される型があるので問題はない。
> カスタムなプロパティは node のインスタンスごとに異なりうるので、その場その場で
> 指定される値は**全部文字列として扱って構わない**。入力の値から型を推論して決めた
> ところで、**その型を使う場面が無い**ので使い道がない。

**私が取り違えていたのは「型の出どころ」である。**型は**宣言**の側にあり、値の側には
無い。値から推論すると、推論した型を使う場所がどこにも無いばかりか、実害が出る —
入力から型を決めて値を寄せると、`3` と打っただけで数値になり、**文字列の `"3"` を
入れる手段が無くなる** (step2 に型を指定する口は無い)。実機の判定がこれを捕まえた。

**併せて出た設計上の要点**: 拡張が定義する属性には、**型を持ちつつ値をユーザが
上書きできる**ものもありうる。したがって**型の出どころと編集可否は別の軸**であり、
`PropertyRow` が `type` と `readOnly` を別に持つ形はそのまま正しい。

**step2 の帰結**: 型の宣言の仕組みは作らない。編集できるプロパティはすべて custom
なので、型は**文字列**である。一覧に「文字列」と出すのは推測ではなく事実である。
`inferPropertyType` は使い道が無くなったので撤去した (Phase 7 の検索結果一覧も同じ
規則に揃えた)。宣言から型を引く形は step 3。
...
> 積まない。
>
> **T8 (上書きの報告) は完了 (2026-09-08)。**通知の向きが LWW の勝者側に寄る問題を、
> 検出条件の補集合を別系列で出すことで埋めた。ここでも**通知に出る名前**が効くので、
> node label の効きは競合と報告の両方にかかる。
...
> **設計文書に分割済** (2026-09-18): [step2-phase6-dtr](./step2-phase6-dtr.md)。
> **着手時にコードを読んで、計画に無い事実が 5 つ出た** (DtR の sheet がタブに並ぶ /
> 判断ログに op を足すと古い版が batch ごと捨てる / U7 の 7 種類は既存の 3 カテゴリに乗る /
> merge の cancel は今の器では表せない / fork からの起動は凍結記述だけが頼り)。
> **cancel は先送りし、機構を先に作って見せ方は骨だけにする** (利用者判断 2026-09-18)。
...
- **呼び出し対象の既定値は非対称**である。explicit merge = 共同作業者全員 /
  ...
  見て対象を追加・削除できる。**後者は fork に凍結された競合の記述から引く** (Phase 3) —
  畳み直して求めるのではない。その時にはもう競合が消えていることがある
...
**Exit**: いま表示しているシート (trunk / branch のどちらでも) の label・content・property を
部分一致で引け、結果の 1 件からグラフの該当要素へ到達できる。

#### 実装 (2026-09-20)

| スライス | 内容 | commit |
| --- | --- | --- |
| S1 | 検索の中核 (`src/client/src/search/searchSheet.ts`)。`propertyCategory` / `inferPropertyType` を `properties.ts` に追加 | `7167c89` |
| S2 | 検索窓と結果一覧 (`SearchPanel.tsx`)、`GraphEditor` への結線 | `732fe64` |

**仕様どおりにしなかった点が 2 つある。**どちらも意図的で、step 3 で見直す。

- **ダイアログを移動可能にしていない。**仕様は「ダイアログは移動可能」と求めるが、
  **既存の 9 つのダイアログに移動できるものが 1 つも無い** — すべて `inset: 0` の覆いに
  中央寄せである。移動を作ると覆いの構造から変えることになり、UI/UX の見直し
  (step 3 へ繰延) に食い込む。代わりに**覆いを敷かず隅に浮かべる**ことで、
  「結果を見ながらグラフを触る」という移動可能性の目的の方を満たした
- **Enter を待たずに引く。**仕様は検索窓の要素として enter を挙げているが、打つたびに
  結果が出る方が掴みやすい。Enter でも引けるので仕様を狭めてはいない

**型の推論を Phase 4 と共有する形で置いた。**仕様 `propertyEditor.md` が「step 2 では型を
値から推論して表示するだけ」「**型は値の従属変数である**」と定める以上、型は保存されて
おらず、property editor (Phase 4) と検索の結果一覧 (Phase 7) が同じ推論を要る。二度書くと
片方だけ直す事故になるので `properties.ts` に 1 つ置いた。**Phase 4 はこれを使う。**

#### trunk / branch を切り替えると検索は初期値に戻る (利用者判断 2026-09-20)

効いているのは **`App` が `GraphEditor` に渡す `key`** (`${activeSheetId}/${branchId}`) で、
切り替えるとコンポーネントごと再マウントされる。**結果だけでなく検索窓も閉じる。**

trunk と branch は別のグラフなので、**移った時点で結果は無効**である。窓だけ残って空の
結果を出すより素直だと判断した。窓の開閉を `App` へ上げれば再マウントをまたいで残せるが、
状態を `GraphEditor` の外に出すことになり最小 UI の約束から出るので採らない (→ step 3)。

**⚠️ この経路は当初、私が入れた effect (`[file.id, activeSheetId]` で結果を捨てる) が
担っていると思い込んでいた。実際には一度も発火していない** — `activeSheetId` が変われば
`key` が先に再マウントするので、effect は常に no-op だった。**死んだ安全網は、動いている
安全網より悪い** (テストされないまま「守られている」と読めてしまう) ので取り除いた。

**見落としの形は Phase 6 の 💬 ボタンと同じである。**実機の項目が `li button` を数える
だけで「結果が消えた」と「窓ごと消えた」を区別しておらず、**スクリーンショットを見て
初めて分かった**。通った項目が、狙った経路を通ったとは限らない。
...
| **U7** | **resolve graph の UI**。**分類は決着 (2026-09-18, Phase 6 の事実 C)** — 別枠に置いた 2 つ (`node.setParent` / `edge.reconnect`) は `isParallelStructureOp` に含まれ、**既に structure の並行変更として検出されている**。resolve graph は新しい分類を作らず **既存の 3 カテゴリ (content / structure / layout) の上に乗る**。残るのは見せ方 (→ Phase 6 の V2)。以下は当初の記述: 仕様は「trunk の上に競合を表示する」までしか決めていない。競合の種類は **7 つ** — 対立として出る 5 つ (edge label / edge property / node label / node 内容 / node property) に加え、**「conversensus 側で解決したが、ユーザの意図に合わない可能性があるもの」2 つ** (edge の接続先、グループの所属関係。merging.md の S3 / S5 に対応) を表示する必要がある。さらに **resolve graph のすべての要素が追加/削除/編集可能**でなければならない (競合を避けるために既存の他の要素を変える必要が生じうるため)。dialogue graph からは要素を **id で指す** (URI は step3) | Phase 6 |
...
## 5.5 仕様に戻すべき論点 (2026-08-30 のレビューと、その PR のコメントで出たもの)
...
| **S4** | **同じ競合から fork がいくつできるか (未決)。** implicit merge は全参加者がそれぞれの手元で行う導出なので、**同じ競合を全員が独立に検出する**。素直に作ると一つの競合に参加者の数だけ fork ができ、人間からは同じ理由の fork が並んで見える。仕様は「merge できない場合には fork し、actor には通知する」としか書いていない。**根は「fork は判断だから書く」という理由づけの不正確さにある** — fork が作られること自体は自動であって、書くのは解決という判断が取り消す対象を必要とするからにすぎない。**推奨**: fork の同一性を競合そのもの (対象の id、プロパティ名、対立した二つの op の batchId) から導く。これらは誰の手元でも同じ値になるので、独立に書かれた fork は畳み込みの段階で一つにまとまる。凍結した記述が食い違っていても (名簿の食い違いで見えている範囲が違えば起こりうる) 同一性は変わらない |
| **S5** | **「保留」の意味が実装と噛み合っていない (未決, 2026-09-18)。** `dialogueToResolveGraph.md` は「保留の間, trunk には merge されない. branch はそのまま生き続ける」と書くが、これは **merge がまだ適用されていない前提**である。Phase 3 の決定 ② で **merge は押した時点で適用される** (事前検査 + 人の確認。保留の器は作らない。決め手は trunk op-log に revert が無いこと) と決めたので、実際の保留は **「trunk には既に入っているが、決着していない」**状態になる。**利用者から見た意味が違う** — 「待つ間は安全」ではなく「入った後で話している」。cancel を先送りすると決めた (Phase 6) ので、この読み替えは仕様側に反映する必要がある |

...
### ⚠️ 移行だけでは足りない (2026-09-03, dev 環境で発覚)

**移行は 1 回きりなので、その後に作られた File には誰も起点を置かない。**
実機で「招待しても表が空のまま」になった。招待レコードは PDS に 3 件書けており、
起点だけが無かった。起点が無い File では招待が 1 件残らず `issuerNotParticipating` で
捨てられ、名簿は永久に空になる。しかも**捨てられた招待は一覧に行を持たない**
(`rosterRows` が `invalid` として出すのは捨てられた**承認**だけで、招待された人は
名簿のどこにも現れない) ので、画面にも console にも何も出ない。

起点を置く経路を 3 つに増やす。

1. **File 作成時** (`handleCreate`)。**作成のその瞬間だけが「誰が作ったか」を知っている**。
   これが本筋で、移行は step1 の File のための後追いにすぎない
2. **名簿を読むたび** (`ensureOwnGenesis`)。ログアウト中に作った File は 1 で置けないので、
   拾い直す口が要る。**自分だけで書かれた op-log の File に限る** — 起点は
   「この File を作ったのは自分だ」という主張なので、確かめずに書いてはならない
3. 起動時の移行 (上記)。step1 の File のため

**後から置けるのは genesis の clock が 0 に固定されているからである** (`JUDGMENT_GENESIS_CLOCK`)。
0 は誰にも割り当てられないので、既に書かれた招待より必ず前に来る — 起点を後から置くと、
それまでに書かれた招待が**そのまま有効になる**。

**捨てた判断は画面に出す** (`rejectedNote`)。この欠陥が実機まで残ったのは、
名簿の畳み込みが理由を返しているのに UI がそれを 1 つも出していなかったからである。

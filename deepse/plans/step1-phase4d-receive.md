...
| **4d-2** ✅ | 端末一意 `actor` = `did#deviceId` (§3.1)。deviceId の生成・永続化 (`sync/actor.ts`)。**actor の供給を UI の `GraphEvent` から同期層 (`EventSyncTap`) へ移す**。`LOCAL_ACTOR` と、消費者が無くなった `GraphEvent.userId` を削除 | なし |
| **4d-3** ✅ | 順序規則: `observe` の配線 (§3.2a) + tiebreak を `clock → actor → id` へ (§3.2b)。単一 actor での同順序を回帰テストで固定。`foldFileStructure` は現挙動をテストで固定するのみ。**受信経路が未存在のため `observe` は入口 (`EventSyncTap.observeRemote`) までとし、実呼び出しは 4d-5** | なし |
| **4d-4** ✅ | `pull` の cursor を**廃止** (§1.3 の当初案「レコード順ベースへ」は成立しなかった — 下記 §8 参照)。`pull(since)` → `pullRemote()` で常に全件返し、既読位置を持たない契約にする。`maxClock` の問題は cursor ごと消滅。`subscribe` の既読管理も id 集合へ (§1.5 の baseline 欠陥も同時に解消)。`catchUp` の fileId フィルタ (D-6, 4d-1 から繰延) もここで実施 | なし |
| **4d-5** ✅ | 受信経路の配線 — remote pull → **`POST /files/:id/batches/received`** 直書き (§3.3)。3 つの不変条件を満たす。**書き込み先は既存の `POST /files/:id/batches` ではなく新設エンドポイント** — 既存の口は marker を立てないため §3.3b を破る (下記 §8 参照) | なし |
| **4d-6** ✅ | 実機 e2e (device A/B)。**基準 1〜4 は PASS、基準 5・6 は 4d のスコープでは原理的に達成不能と判明し Phase 4e へ移した** (結果と根拠は §9) | **あり** |
...
> **2026-07-21 追記 (4d-6 実施後)**: 下記のうち **基準 5・6 は Phase 4e へ移した**。
> 受信側は genesis batch (= `sheet.create` の唯一の在り処) を remote 経由で受け取れないため、
> 4d の範囲では原理的に満たせないことが実機で確定した。**4d の完了判定は基準 1〜4**。
> 詳細と根拠は §9。
...
検査は **`scripts/inspect-local-oplog.ts`** で行う (W3d5-7 の `inspect-remote-batches.ts` が
PDS 側 = 送信結果を見るのに対し、こちらがローカル正典側 = 受信結果を見る。拡張ではなく
別スクリプトにした理由は §8)。基準 6 の判定ロジックは `src/shared/src/events/applicability.ts`
に置き、単体テストで固定してある。

実行例 (device B を検査する):

```bash
DAEMON_URL=http://localhost:3001 PEER_URL=http://localhost:3000 \
  DATA_DIR=<device B の data ディレクトリ> PDS_URL=http://localhost:2583 REPO=<handle> \
...
- **2026-07-20 (4d-2 実装): actor の供給元を `GraphEvent.userId` から同期層 (`EventSyncTap`) へ
  移した。** 設計 §4 は「`GraphEvent.userId` 起点」と書いていたが、**実装できない** —
  `makeEventBase` はセッションに触れない純関数で UI の各所から呼ばれるため、そこで
  `did#deviceId` を組み立てるにはモジュールレベルの可変状態 (グローバルな「現在の actor」) が
  必要になり、テストしづらく壊れやすい。actor は「誰が編集したか」という UI 上の属性ではなく
  **Lamport の因果順序と重複排除の単位を識別する同期層の識別子**なので (§1.1)、同期層が
  与えるのが素直だった。
  - `graphEventToBatch(event, { clock, actor, sheetId? })` にシグネチャ変更。
  - これにより `GraphEvent.userId` は**唯一の消費者を失った**ので削除した (`LOCAL_USER_ID` も)。
    残すと新たな dead field になり、`LOCAL_ACTOR` を消したのと矛盾する。
  - deviceId は初回起動時に UUID を生成し localStorage へ。クリアされたら新端末として振る舞うが
    **正しさは失われない** (actor が 1 つ増えるだけ) ので再生成を防ぐ仕組みは持たない。
...
- **2026-07-21 (4d-3 実装): `observe` の配線は「入口まで」に留めた。** 受信経路そのものが
  4d-5 まで存在せず、実呼び出し元を作れないため。`EventSyncTap.observeRemote(remoteClock)` を
  生やして Lamport 受信規則の挙動を単体テストで固定し、実呼び出しは 4d-5 で繋ぐ。
  スライス分割 (§4) の趣旨どおり 4d-5 を「配線するだけ」に保てる。
- **2026-07-21 (4d-3 実装): tiebreak から `timestamp` を外した** (`project.ts:orderBatches`)。
  `clock → timestamp → id` → `clock → actor → id`。`Batch.timestamp` フィールド自体は
  表示用として残す (削除しない) が、**順序付けには一切使わない**ことをコメントで明記した。
  `GENESIS_CLOCK_START` のコメントも更新 — 一意連番の理由が「timestamp への昇格回避」から
  「genesis は全 batch が `GENESIS_ACTOR` を共有するので、clock が一意でないと順序が
  ランダム UUID の id で決まってしまう」に変わったため。
- **2026-07-21 (4d-3 実装): `orderBatches` の影響分析はリスク HIGH** (gitnexus, upstream)。
  直接の呼び出し元は `projectBatches` / `projectFile`、間接に `branchLog.branchSheet` と
  `EventStore.projectSheet`。影響する実行フローは 3 つ。**単一 actor での退行なしを
  回帰テストで固定**した上で変更した (§3.2 のとおり `tick()` の単調性から構造的に言える)。
  既存 528 テストは全て緑で、`timestamp` 差に依存していたテストは 1 件も無かった
  (テストヘルパ `batch()` が `timestamp = clock` を既定にしていたため)。
- **2026-07-21 (4d-3 実装): `foldFileStructure` は無変更のまま現挙動をテストで固定した** (§3.2)。
  `sheet.setName` の「整列後の最終適用が勝つ」(LWW ではない) と `sheet.reorder` の
  「後勝ちでマージしない」を明示的なテストにした。4e で改善するときの回帰検出点になる。
- **2026-07-21 (4d-4 実装): §1.3 の「cursor を remote のレコード順ベースへ変える」は
  成立しなかった。cursor そのものを廃止した。** 実コードと `@atproto/api@0.19.6` の型を
  当たった結果、§1.3 が挙げた 3 候補がいずれも現状の rkey 方式では使えない:
  - **`listRecords` の cursor は rkey 位置**。本実装の rkey は batchId (ランダム UUID)
    なので順序が時系列にならず、後から書いた batch の UUID が保存済み cursor より小さいと
    永久に取りこぼす。**§1.3 が clock cursor を否定した理由「端末をまたぐと単調でない」が
    そのまま rkey にも当てはまる — 同じバグの構造**。
  - **`indexedAt` は repo の `listRecords` 出力に存在しない**。出力は `{ uri, cid, value }`
    のみ (型で確認)。`indexedAt` は appview 側の概念。
  - **`rev` はレコード単位では露出しない**。`com.atproto.sync.*` (firehose / getRepo) が要る。
  → **既読位置を持たない契約にした** (`pull(since)` → `pullRemote()`)。取りこぼしゼロを
  構造的に保証し、二重取り込みは 4d-0 の `appendReceivedBatches` のべき等性が無害化する。
  `maxClock` が返却対象外を算入する問題 (§1.3 後段) は cursor ごと消滅した。
  - **代償は毎回 O(全履歴) の list**。ただし起動契機は起動時 + `online` + 手動に限られる
    (§3.4 で subscribe を不採用としたため) ので受容できる。W3d5 の catch-up が既にこの
    コストで動いており、実機で問題になっていない。
  - **rkey を時系列ソート可能なキー (TID) へ変える案は Jetstream 化と同じ Phase へ送る**
    (ユーザー決定)。rkey=batchId のべき等性 (putRecord 上書き) を失い batch.id を
    レコード body へ移す必要があるため、4d-1 級のスライスになる。§3.4 が既に subscribe を
    その Phase へ送っているので合流先として自然。`list()` のページングも同時に扱う。
- **2026-07-21 (4d-4 実装): `subscribe` の既読管理を cursor から観測済み id 集合へ変えた。**
  cursor 廃止に伴う必要変更だが、**§1.5 の欠陥修正を兼ねる** — cursor 版は baseline 確立が
  失敗すると次の成功 poll が baseline になり、その間の batch を恒久的に落としていた。
  id 集合なら poll が失敗しても集合は前進しないので取りこぼしが残らない。回帰テストで固定した。
  消費箇所は 0 件のままで、Jetstream 化での作り直しまでの繋ぎ。
- **2026-07-21 (4d-4 実装): `catchUp` の fileId フィルタ (D-6) を実施した。** 4d-1 で
  繰延した前提条件 (「`pull` が fileId を返せること」) が `pullRemote(): Promise<RemoteBatch[]>`
  で揃ったため。**直していたのは正しさではなくコスト** — batch id は UUID なのでファイルを
  跨いで衝突せず誤マッチは起きていなかった。無関係な全件を毎回舐めるコストのみが問題だった。
- **2026-07-21 (4d-4 実装): `RemoteBatchTarget` から `SyncProvider` 由来の `pull` を外した。**
  `pullRemote(): Promise<RemoteBatch[]>` に置き換え、送信 (`pushRemote`) と対称にした。
  4d-1 で `push` → `pushRemote` としたのと同じ理由 — `SyncProvider` はファイル単位の境界、
  remote の batch コレクションは repo 全体。取得側も同じ非対称を型に出すべきだった。
  影響は封じ込められている: `AtprotoSyncProvider.pull` の呼び出し元は `RemoteSyncQueue.catchUp`
  と自身の `subscribe` のみで、`FanoutSyncProvider.pull` / `EventSyncTap` の `pull` は
  **local 側**を指している (gitnexus impact: リスク LOW, 直接の呼び出し元 0)。
- **2026-07-21 (4d-5 実装): 受信の書き込み先を新設エンドポイント
  `POST /files/:id/batches/received` にした。** 設計 §3.3 は「受信 batch は
  `POST /files/:id/batches` へ書く」と書いていたが、**そのまま従うと §3.3b の不変条件を破る** —
  既存エンドポイントは `appendBatches` (marker を立てない) を呼ぶため、書いた直後の
  `GET /files/:id/batches` が lazy migration を起動し受信内容を破棄する (§1.8)。
  4d-0 で `EventStore.appendReceivedBatches` を作ったが **HTTP へ露出していなかった**ことに、
  4d-5 の配線時点で気づいた。
  - **クエリパラメータ (`?received=true`) ではなくエンドポイントを分けた**。ローカル編集の
    書き込み経路 (hot path) を一切触らずに済み、marker を立てる/立てないの区別が
    ルーティング表に現れる。取り違えは経路で防ぐのが確実。
  - 回帰テストは W3d-1 の「初回 read 前に積まれた pre-W3 増分は migration で破棄される」と
    **同じ手順で書き込み口だけを替えたもの**にした。同じ状況で結果が逆になることを固定し、
    かつ通常 POST 側の破棄挙動が壊れていないことを対照テストで示す。
- **2026-07-21 (4d-5 実装): 受信のスコープは「開いているファイル 1 つ」に限定した** (§2)。
  `pullRemote` は repo 全体を返すので、`receiveRemoteBatches` が fileId で絞る。
  **このフィルタが §1.11 D-4 (孤児 batch) の防御を兼ねる** — 未知の fileId を書き込むと
  ファイル一覧に出ない batch が生まれるが、開いているファイル以外を書かないので起きない。
  新規ファイルの跨端末伝播は Phase 4e のまま。
- **2026-07-21 (4d-5 実装): `observe` は書き込み成功後に呼ぶ。** 取り込めていないのに
  clock だけ進むと、次に発番する batch が「取り込めなかった編集より後」を騙る。
  失敗時に `observeRemote` が呼ばれないことをテストで固定した。
- **2026-07-21 (4d-5 実装): 受信と送信 catch-up は同じ `useEffect` に相乗りさせ、
  catch は独立にした** (§3.4)。両者は「remote と突き合わせて差分を埋める」同じ性質の
  操作なので発火経路を分けない。ただし送信の失敗が受信を止めない (逆も同様)。
- **2026-07-21 (4d-5 実装): テストで `mock.module('../api', ...)` を使ってはいけない。**
  bun のモジュールモックは**グローバルに効く**ため、他のテストファイルから `../api` の
  別の export が見えなくなる (実際に `createFile` not found で別ファイルが落ちた)。
  既存の `createLocalProvider` と同じ**注入の型** (`appendReceived` オプション) に揃えた。
- **2026-07-21 (4d-6 準備): 検査は `inspect-remote-batches.ts` の拡張ではなく、
  対になる `inspect-local-oplog.ts` を新設した。** §5 は「W3d5-7 のスクリプトを拡張して
  ローカル op-log 側も検査できるようにする」と書いていたが、両者は**データ源も判定内容も
  共有しない** — 既存は PDS の listRecords を見て送信結果 (genesis 非 push・presentation
  非搭載) を判定し、新設はデーモンの `GET /files/:id/batches` を見て受信結果を判定する。
  1 ファイルに畳むと「どちらのモードか」の分岐が全検査に伝播するので、送信側/受信側で
  スクリプトを分けた。
- **2026-07-21 (4d-6 準備): 受入基準 6 の判定ロジックはスクリプトではなく shared の
  ドメインモジュール (`events/applicability.ts`) に置き、単体テストを付けた。**
  「op が projection へ効いたか」は `projectFile` / `applyOp` の畳み込み規則を写した
  **第 2 の実装**であり、自明ではない。ここが誤ると 4d-6 の実機検証そのものが嘘をつく
  ので、実機に持ち込む前に単体で固定する必要がある。`orderBatches` を `project.ts` から
  export したのは、判定が本物と**同じ順序**で走ることを構造的に保証するため。
  - **時点判定と最終判定を使い分ける**必要があった。content op の宛先シートは
    `projectFile` が全 batch 畳み込み後の構造でグルーピングするので**最終 live 集合**で、
    `sheet.setName` 等は `applyFileOp` が畳み込みの途中で `if (meta)` を見るので
    **op 時点の live 集合**で判定する。取り違えると「作成 → 改名 → 削除」という正常な
    操作列が赤くなる。
  - **落ちた op と、落ちてはいないが届かない op を分けた**。対象不在の layout/style は
    `applyOp` が対象の有無に関わらず Map へ書き込むので**落ちていない** (孤児として残る)。
    不在対象への remove も冪等な削除で異常ではない。両者を FAIL にすると実機が
    ノイズで赤くなり、本当の欠落が埋もれる。warn として別枠で報告する。
- **2026-07-21 (4d-6 準備): 基準 3 (取りこぼし) は「受信件数 = 送信件数」ではなく
  **batch id の包含**で判定する。** §5 は件数比較を代替案として挙げていたが、ローカル
  op-log には remote に載らない genesis batch が含まれるので件数は元から一致しない。
  「PDS 上の当該 fileId 宛 batch の id が、すべてローカルにある」なら件数一致より強く、
  どの batch が欠けたかも分かる。
- **2026-07-21 (4d-6 準備): 基準 2 (べき等) は `--snapshot <path>` による 2 回実行の
  比較にした。** 単発実行では「2 回受信しても変わらない」を判定しようがない。1 回目で
  batch 数と projection の指紋を記録し、再受信後の 2 回目で比較する。
  - **指紋から layout / presentation を除いた**。presentation はローカル限定で同期対象外
    (`isSyncable`) なので、含めると収束していても基準 5 が不一致になる。
- **2026-07-21 (4d-6 準備): 検査基盤を PDS 非依存の 2 デーモン構成で先に検証した。**
  `POST /files/:id/batches/received` へ直接投げて device A/B を模擬し、(a) 正常系が全 PASS、
  (b) 状態が変われば基準 2 が FAIL、(c) **シート作成 batch を伴わない content batch が
  `appended: 1` で着地しつつ基準 6 で FAIL する**ことを確認した。(c) は §1.10 の
  bootstrap ギャップそのもので、**基準 1「op-log に行が増えた」なら PASS してしまう**
  ケースが実際に存在することの実証になっている。実機で初めて検査器を動かすと、
  FAIL が実装の問題か検査器の問題か切り分けられない。

---

## 9. 4d-6 実機 e2e の結果 (2026-07-21)

構成: PDS `:2583` (alice.test 共有) / device A = daemon `:3000` + client `:5173` /
device B = daemon `:3001` + client `:5175`。**同一 DID の 2 端末** (actor は
`did:plc:jice...#97a7255b` と `#1af696c0` で端末まで一意に分かれた — 4d-2 が実機で成立)。
検証ファイルは新規作成 (`ce34a816`)。既存 PDS に残っていた W3d5 期の 7 レコードは
`fileId` 無しなので counted skip され、混入しなかった (4d-1 が実機で成立)。

### 判定結果

| 基準 | device A | device B | 備考 |
|------|---------|---------|------|
| 1 受信着地 | PASS | PASS | 双方の op-log に 2 端末分の actor が載った |
| 2 べき等 | PASS | PASS | 再受信で batch 数・projection ともに不変 |
| 2 marker | PASS | PASS | §1.8 の破棄は起きていない (4d-0 が実機で成立) |
| 3 取りこぼし無し | PASS | PASS | PDS 上の 3 件がすべて両端末に届いた |
| 4 ローカル編集が壊れない | PASS | PASS | B 自身の編集 (clock=6) は A の受信後も残存。**ただし undo は未検証** |
| 5 収束 | **FAIL** | **FAIL** | 下記 |
| 6 適用不能 op 0 件 | PASS | **FAIL** (5 op 中 5 件) | 下記 |

### 🔴 基準 5・6 は 4d のスコープでは原理的に達成できない

device B の受信 op はすべて `unknown-sheet` で落ちた。原因は §1.10 が予測したとおりで、
実機は予測を裏付けた **— が、設計はその帰結を受入基準へ反映し損ねていた**:

- genesis batch は remote に載らない (`remoteFilter.ts:34`, C1 の不変条件)。
- `sheet.create` は genesis batch にしか入っていない (ファイル作成時に発番されるため)。
- ゆえに **受信側は op-log 経由でシートの存在を知る手段が原理的に無い**。
- 結果、content batch は着地するが `projectFile` のグルーピングで全件捨てられ、
  B の projection は空のまま → 基準 6 が落ち、必然的に基準 5 (収束) も落ちる。

**これは実装の不具合ではなく、§2 が bootstrap を Phase 4e へ送った結果である。**
つまり §5 の基準 5・6 は **4e の成果物を前提とする基準**であり、§2 (非目標) と
矛盾していた。起案時にこの矛盾に気付けなかったのは、§1.10 を「気付きにくい失敗モード」
として扱い、「**第 2 端末では必ず起きる**」構造的必然として扱わなかったため。

→ **4d の完了判定は基準 1〜4 とする**。基準 5・6 は Phase 4e の受入基準へ移す。
基準 6 の計測器 (`applicability.ts`) は 4e でそのまま使う。

### 副産物: legacy snapshot 経路の肩代わりを実機で再確認した

device B の画面には "A が作ったノード" が**正しく表示されていた** — op-log 正典が
空であるにもかかわらず。legacy `app.conversensus.graph.file` 経路が肩代わりしている。
W3d5 critic A2 の「画面を証拠にするな」が、受信でも同じ形で成立することの実証。
**画面・op-log 行数のどちらを見ても PASS に見え、基準 6 だけが落ちた**ことになる。

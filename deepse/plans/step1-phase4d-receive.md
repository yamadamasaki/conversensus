...
**対策 (4d-0)**: `migrateToOplog` の tx 内 re-check に
**「既存 batch が 1 件でもあれば migration せず marker だけ立てる」**を追加する。
破棄の前提 (「snapshot が正典」) は「**op-log が空**」でこそ担保されるべきで、
marker 単独は受信を想定していない古い前提である。
...
| スライス | 内容 | PDS 依存 |
|---------|------|---------|
| **4d-0** | **migration ガード** (§1.8)。`migrateToOplog` の tx 内 re-check に「op-log が空でなければ migration せず marker だけ立てる」を追加 | なし (server 単体) |
| **4d-1** | `BatchRecord.fileId` 必須化 + mapper 往復。`fileId` 無しレコードは counted skip (§3.1)。`catchUp` の fileId フィルタ (§1.11 D-6) | なし |

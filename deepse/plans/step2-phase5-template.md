...
- ~~**toulmin edge の繋ぎ替えに制約をかけていない**~~ **解消した (2026-09-13)**。
  仕様の「edge の種類が変わらない範囲でのみ接続を変更できる」を
  `canReconnectByTemplate` で実装した。`isValidConnection` は `Connection` しか渡さず
  付け替え中の edge が分からないので、`onReconnectStart` で控えて渡す。
  **当たっている template で種類を解決できない edge は制約の対象外**とした — 名前だけで
  判定すると、相手の template を持たないせいで付け替えが塞がる (P3 と同じ判断)。
  実機 (Chromium) で、種類が変わる付け替え (データ → 論拠) は `edge.reconnect` が増えず、
  種類を保つ付け替え (主張 → 主張B) は増えることを確認した

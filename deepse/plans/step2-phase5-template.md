...
| **P4a** | **P4 の手直し** (2026-09-12 の仕様変更)。種別を `properties.kind` に載せ、右クリックの変更を撤回する | 単体 + 実機 |
| **P5** | edge の種類を**両端から決める** (`edgeKindsBetween` + 候補 1 なら自動)。メニューは作らない | 単体 + 実機 |
| **P6** | 候補 0 の接続を**拒否**する (`isValidConnection`)。toulmin node 間のみ | 単体 + 実機 |

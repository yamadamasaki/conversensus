...
**export / import は非目標ではない** (レビュー D1 で追加, 2026-08-12)。`.conversensus` は
v4 まで base64 で自己完結していたので, 参照だけにすると**別端末で開いた瞬間に画像が
失われる** = 自己完結性の回帰である。**v5 で実体を `blobs` 欄に同梱する**ことにした。
op-log には base64 を戻さないので, 膨らむのは配布物であるファイルだけである。
実体がこの端末に無い参照 (他端末が作って未表示の画像) は運べないので, その場合は
書き出し時に利用者へ伝える。

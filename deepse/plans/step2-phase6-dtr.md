...
| **D3** | **再 merge**。承認が揃ってから。再び競合したら繰り返す。**前半 (器) と中盤 (表示の門) は完了 (2026-09-19)**。① 解決グラフの器を branch にする (決めたこと 10) ② `Commit.dtrId` + `MergeBranchParams.dtrId` で「どの DtR の決着か」を刻む ③ `admissibleBatches` が承認を経ていない再 merge の写しを **projection の手前**で落とす ④ tap が判断ログから `foldDtr` し、ref 経由で trunk 表示 2 箇所に届ける。⑤ `sync/remergeDtr.ts` が**関門と刻印**を足して `mergeBranchOnOplog` に委譲する。**完了 (2026-09-19)**。

**関門は 2 つあり、効かせる場所が違う** — これが D3 の設計の核心である。

| 条件 | どこで | なぜ |
| --- | --- | --- |
| 承認が揃っている | **書く側と畳み込みの手前の両方** | trunk を書き換えるので端末をまたいで一致しなければならない。早まって書いても `admissibleBatches` が落とす (二重の守り) |
| その後の変更がない | **書く側だけ** | 仕様は「再 merge が**可能になる**条件」として書いており、捨てる pre 条件としては書いていない。判定に解決 branch の先端が要るので、表示のたびに別のログを読むことになる。委ねなくて安全なのは、**変更があったのに書いても承認の条件は畳み込みが判定する**からである |

**⚠️ 同着の向きが 2 つの述語で逆である。**`canRemergeAt` は「承認がこの操作より前か」を問うので同着を**通さない**。`canStartRemerge` は「承認の後に動いたか」を問うので同着は「動いていない」= **通す**。同じ clock の比較だが向きが違う (変異試験で両方を固定した)。

**新しい merge 機構は作っていない。**再スタンプ・べき等性・競合検出は既存の経路のまま。「再び競合したら繰り返す」にも仕掛けは無く、返る `conflicts` が非空なら DtR をもう一度起こせばよい。

**残り: 起こす口 (UI) と配線のテスト。**`useFileSheetOperations.test.ts` は `syncRecord` を差し替えて内部 tap を迂回しており `onRoster` が鳴らないので、通しのテストは D5 (見せ方の骨) で入れる | 単体 |

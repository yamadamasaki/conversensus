...
| **D3** | **再 merge**。承認が揃ってから。再び競合したら繰り返す。**完了 (2026-09-19, 3 コミット)**。① 解決グラフの器を branch にする (決めたこと 10) ② `Commit.dtrId` + `MergeBranchParams.dtrId` で「どの DtR の決着か」を刻む ③ `admissibleBatches` が承認を経ていない再 merge の写しを **projection の手前**で落とす ④ tap が判断ログから `foldDtr` し、ref 経由で trunk 表示 2 箇所に届ける ⑤ `sync/remergeDtr.ts` が**関門と刻印**を足して `mergeBranchOnOplog` に委譲する。詳細は下の §6a。**残り: 起こす口 (UI) と配線のテスト** → D5 | 単体 |
| **D4** | **fork からの起動** (implicit)。既定の呼び出し対象は自分だけ。材料は凍結記述 (事実 E)。**完了 (2026-09-19)**。D1 と**入口だけが違う**ので、器を 2 つ作って `dtr.open` を書く共通部分を `openDtr` に括り出した (分けて持つと片方だけ直す事故が起きる — T0 で踏んだ形)。違いは 3 点: ① **線引きをしない** — D1 は自動起動なので `needsForcedStart` で content に絞るが、こちらは**人が通知を見て押す**。種別で拒むと「通知に出ているのに起動できない」という説明のつかない状態が生まれる ② **既定値は凍結記述から引く** (`calleesFromFork`) — 仕様 `merging.md` が「凍結記述の唯一の用途は、この fork から DtR を起動するときの**呼び出し対象の既定値を供給する**こと。既定値の供給源であって呼び出し対象そのものではない点は名簿と同じ」と定める。**畳み直さない** (事実 E: 畳み直すと「今」の競合になり、消えていることがある)。`actor` は端末単位なので **DID へ落とす** — 落とさないと 2 台目が別人として数えられ承認が永久に揃わない ③ 解決 branch は **fork のシート**から切る。**残り: 起動の口 (UI)** — 通知は既に `ConflictNotice` に「相手が保留した競合」として出ており、そこに押す口を足すのが D5 | 単体 |
| **D5** | **見せ方の骨** (V2 / V4)。サイドバーに branch と同じレベルで出し、通常の branch と違うと分かる形。DtR の sheet はタブに出さない (事実 A)。**実装完了 (2026-09-19)**。4 点: ① 起動後に `branchReceiveEpoch` を進めて解決 branch を一覧へ (T7-3 の機構の再利用 — `startDtr` の中で branch を切るので `handleCreateBranch` の経路を通らない) ② 解決 branch の記号を `⎇` → `⇄` に変える ③ dialogue sheet をタブ一覧から除外 ④ `ConflictNotice` の fork の行に「対話を始める」の口 (**ログイン中だけ** — 判断ログの書き先が自分の repo なので)。**⚠️ 判別は名前の接頭辞という仮のもの** (利用者判断 2026-09-19: 最小コスト優先)。本来は `dtr.open` の記録から引くべきで、**名前は人が変えられる**。接頭辞の定数と述語を `startDtr.ts` (名前を作る場所) に閉じ込めてあるので、UX を作り直すときに剥がす範囲はそこ 1 つ | 単体 + 実機 |
| **D6** | 2 アカウントの実 PDS で通しで確認。**完了 (2026-09-19)。Exit 6 項目すべて PASS、無言の失敗 0 件** (alice :3100/:5174 と bob :3200/:5176、PDS :2583)。D5 の見せ方も実機で効いていた — 解決 branch が bob 側に `⇄` の印つきで現れ、対話用シートはタブに出ない (`sheet.create=2` に対し画面の DtR タブ=0)。**⚠️ 留保: 承認は判断ログへ直に書いて判定した** (承認の UI がまだ無いため)。「機構が実 PDS で動く」ことは示せたが「人が画面から承認できる」ことは示していない | 実 PDS |

## 6a. D3 の設計の核心: 関門は 2 つあり、効かせる場所が違う
...
**⚠️ 同着の向きが 2 つの述語で逆である。**`canRemergeAt` は「承認がこの操作より前か」を問うので
同着を**通さない**。`canStartRemerge` は「承認の後に動いたか」を問うので同着は「動いていない」=
**通す**。同じ clock の比較だが向きが違う (変異試験で両方を固定した)。

**新しい merge 機構は作っていない。**再スタンプ・べき等性・競合検出は既存の経路のまま。
「再び競合したら繰り返す」にも仕掛けは無く、返る `conflicts` が非空なら DtR をもう一度
起こせばよい。

## 6b. 利用者の実機確認で出た 2 件 (2026-09-20, 修正済)

**どちらも機構 (配線) 由来で、最小 UI だからではない。**単体テストは両方とも素通りした。

### ① merge した本人にだけ解決 branch が出ない

**原因**: `reloadBranchList` が `branchReceiveEpoch` を進めるだけだった。あれは App が
`fileOps.receiveEpoch` と足して**引数として渡し直す**値なので、**同じレンダーでは届かない**。
しかも直後に `afterMerge` が `existing.map(...)` で一覧を上書きし、**増えた解決 branch を
落とす** (`map` は置換しかしない)。相手側に出ていたのは受信サイクルが別途読み直すためで、
これが非対称の正体だった。

**修正**: 読み直しの実体を `loadBranchList` として切り出し、effect と `reloadBranchList` の
両方が呼ぶ (同じ畳み込みを 2 箇所に書かない)。`handleMergeBranch` では **`afterMerge` の
後**に呼ぶ。定義順の都合で ref 経由にしてある (`reloadBranchList` は `startDtr` の依存配列
から参照されるので前半に置く必要があり、実体は `migrateLegacyMeta` に依存するので後半)。

> **⚠️ D5 のコミットで「T7-3 の機構の再利用」と書いたのは誤りだった。**T7-3 は受信経路の
> 話で、**自分の操作の直後には当てはまらない**。前例を確かめずに当てはめた。

### ② 対話グラフに到達できない

決めたこと 5 に従ってタブから隠したが、**代わりに開く口を作っていなかった**。仕様は
dialogue graph を編集可能なグラフとしているので、**到達不能なのは実装の欠落**である。

**修正**: 解決 branch の行に「対話グラフを開く」(💬) を足し、`dialogueSheetNameOf` で
対になる sheet 名を引いて `onSelectSheet` で開く。対話用 sheet は `projectFile` の結果に
含まれており、隠しているのは Sidebar の `.filter()` 1 箇所だけなので、**到達口さえ作れば
canvas はそのまま描く**。

### なぜ単体テストで捕まらなかったか

- ① は**フック単体では `afterMerge` との競合が現れない**。App を一周する値を同一レンダーで
  期待した誤りなので、App まで含めないと再現しない
- ② はテストが「**隠れていること**」を確かめて満足していた。「**到達できること**」を
  誰も見ていなかった

D6 の台本にも同じ穴があった — 一覧は bob 側 (出ていた方) しか見ておらず、対話グラフへの
到達は試していなかった。**追補 2 件として台本に足した。**

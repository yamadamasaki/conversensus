...
| 2026-08-08 | D4 は仕様変更ではなく**実装を仕様に合わせる修正**だと判明 | 実装後に `operation-manual-for-dev.md` を読み直したところ、ファイル管理・シート管理の両方に「ESC キーを押したり, ダイアログ外をクリックすると保存せずにダイアログを閉じる」と**元から書かれていた**。外クリックで保存していた実装の方がずれていた |

---

## 10. 実機検証の記録 (2026-08-08)

ローカル PDS (`localhost:2583`, `alice.test`) にログインした状態で全スライスを通した。

| # | 見たこと | 結果 |
|---|---|---|
| 1 | 削除時にコンソールへエラーが出ない (ANA-118) | ✅ `ATProto session not initialized` は出ない。残るのは Chrome 拡張由来のノイズだけ |
| 2 | ローカル op-log に tombstone が載る | ✅ clock 3 に `file.remove`。`GET /files` からは消え、`GET /files/ids` には残る |
| 3 | PDS の**最大 rkey が tombstone** になる | ✅ `v1~<fileId>~000000000003~…` が `['file.remove']`。S3 の土台がそのまま観測できた |
| 4 | リロードしても復活しない (層 1, ANA-127) | ✅ 一覧に戻らない |
| 5 | **他端末を模した状態**でも materialize しない (層 2) | ✅ `DELETE /files/:id` でローカル op-log を物理削除し「この端末はこのファイルを知らない」状態を作ってリロード → `[sync] skipped 1 remote file(s) deleted elsewhere` が出て materialize されない |
| 6 | 設定ポップアップの外クリックが破棄になる (ANA-126) | ✅ 名前を書き換えて外をクリック → op-log に `file.setName` が増えず、名前も変わらない |

> 検証 5 が ANA-127 の再現条件そのものである。S3 が無ければ、この状態のリロードで
> ファイルが materialize され「削除したはずのファイルが戻る」ことになる。

副産物: `キャンセル` で削除されないことも確認した (座標クリックの取りこぼしで一度
誤って OK を押してしまい、念のため要素参照で押し直して確かめた)。

...
| (併走 W2) | 実配線 クライアント側: useEventStore ↔ Outbox/SyncProvider (漸進移行の tap)、旧 storage.ts 退役、Lamport clock 永続化 | ✅ 実装済 (下記メモ、退役・clock 復元は残) |
...
### 実配線 W2 (クライアント側) 完了メモ (2026-07-13)

- **アプローチ: 漸進移行 (dispatch に tap)**。既存の `GraphEvent`/undo-redo 機構を残し、dispatch された event を副経路で `Batch` に変換しローカルサーバへ push する。旧スナップショット保存 (PUT /files) と並行の **dual-write** (安全な移行パターン)。読み取り経路の移行と旧保存の退役は後続。
- **成果物**:
  - `src/client/src/sync/eventSyncTap.ts`: `EventSyncTap` (framework 非依存)。`record(event)` で `graphEventToOps` が空でなければ `graphEventToBatch(event, clock.tick())` を Outbox へ enqueue し flush。flush はチェーンで直列化 (Outbox 多重起動回避)、オフライン時は保留維持で次 record が再送。
  - `src/client/src/sync/localServerSyncProvider.ts`: `LocalServerSyncProvider implements SyncProvider`。push=`pushBatches` / pull=`fetchBatches` / subscribe=no-op。宛先はローカル永続デーモン。
  - `src/client/src/hooks/useEventSyncTap.ts`: fileId ごとに tap を生成し `onEvent` コールバックを返す React フック。
  - `src/client/src/hooks/useEventStore.ts`: 任意の `onEvent?` を追加 (後方互換)。dispatch/undo/redo で eventLog に積むたびに呼ぶ。ref 経由で識別子安定。
  - `src/client/src/GraphEditor.tsx`: `useEventSyncTap(file.id)` を `useEventStore` に配線。
- **テスト**: 4 追加 (全 381 pass)。`.test.md` あり。tap のロジックを `RecordingProvider` で決定論的に検証。
- **非破壊**: `onEvent` は任意の副経路で既存挙動不変。既存 UI/保存はそのまま。
- **残作業 (W3 / 後続)**:
  1. **読み取り経路の移行**: ファイル読込を `fetchBatches`→`projectBatches` に切替 (useFileSheetOperations)。検証後に snapshot 読込を退役。
  2. ~~**Lamport clock の永続化・復元**~~: ✅ W3a で実装 (下記メモ)。
  3. **旧 storage.ts / PUT /files の退役**: 読み書き両経路が op-log に移った後。
  4. **presentation の扱い**: 現状ローカルサーバへ presentation op も push (ローカル正典として許容)。remote (ATProto) provider 側で `isSyncable` により除外する。
  5. **高頻度 event (ドラッグ等) の debounce** 検討 (現状は Outbox 直列 flush で吸収)。
- **未検証**: 本環境では実ブラウザ + デーモン起動での end-to-end (実編集が op-log に永続する動作) は未実行。ユニットテスト・typecheck・lint で検証。実機確認は Phase 5 (実証) で行う。

### 実配線 W3a (Lamport clock 復元) 完了メモ (2026-07-13)

- **目的**: 再起動後も操作ログの採番 (Lamport clock) が既存 batch を必ず超えるようにする。W2 では clock が 0 起点で、リロード/再起動のたびに 1 から採番し直すため既存 clock と衝突・逆行し、LWW 順序が壊れる懸念があった。
- **アプローチ: 初回 drain で永続ログの max(clock) を観測して seed**。tap の初回 flush 時に `provider.pull(INITIAL_CURSOR)` で既存 batch を取得し `max(clock)` を `LamportClock.seed` に渡す。以後の `tick` は max+1 から再開する。cursor 文字列ではなく `batch.clock` の最大値を使い provider-agnostic にした。
- **clock 割当を record 時→drain 時へ遅延**: restore は非同期 (pull) だが `tick` は同期。復元前に採番しないよう、`record` は event を保留するだけにし、restore 成功後に drain 内で FIFO 順に `tick` を割り当てて Batch 化する。flush チェーンで直列化されるため採番順は record 順を保つ。
- **成果物**:
  - `src/shared/src/events/unified.ts`: `LamportClock.seed(floor)` を追加。`observe` と違い `+1` せず `value = max(value, floor)` とする (次の `tick` が `floor+1`)。単調性のため現在値より小さい `floor` は無視。
  - `src/client/src/sync/eventSyncTap.ts`: `ensureRestored` (一度きり・失敗時は再試行) で seed。`record` は event を `pendingEvents` に保留し、drain で restore→tick→enqueue→flush。`pending` は保留 event + outbox の合算。
- **restore 失敗時の安全側**: pull が失敗した場合は seed も採番もせず event を保留し、`onError` 通知のうえ次の record で再試行する (0 起点の誤採番を防ぐ)。宛先はローカル永続デーモンなので通常は常に到達可能。
- **テスト**: `LamportClock.seed` 2 件 + `EventSyncTap` の復元/復元失敗 2 件を追加。全 pass。`.test.md` 更新済み。
- **残 W3**: 読み取り経路の移行 (fetchBatches→projectBatches) と旧 storage.ts / PUT /files の退役は引き続き後続。

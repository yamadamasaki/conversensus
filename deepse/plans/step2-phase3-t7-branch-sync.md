...
> **⚠️ これは「ずれ」ではなく不変条件の破れである (2026-09-18)。**含まれるのは
> **merge していない trunk の変更**であり、「branch は分岐点から自分の編集だけを積む」という
> branch の意味そのものを損なう。決定 3 を参照。
...
3. **分岐点は clock の数値のまま** (T7 の範囲では変えない)。
   **⚠️ 2026-09-18 に位置づけを改めた。**当初は「読む人の手元によって分岐点の状態がずれうる」を
   *既知の制約*として記録していたが、実体は **merge していない trunk の変更が branch に映る**
   ことであり、**branch の意味を損なう不変条件の破れ**である。T7-7 後に直した画面の不具合
   (PR #227) は表示だけの話で、**この穴は projection そのものに残っている**。
   解くのは **vector clock — step3 の優先事項** (代案は「分岐時に見えていた batch の集合を
   `base` に持つ」で、実質その部分適用)。`step2-phase3-conflict.md` の未決節と揃えてある

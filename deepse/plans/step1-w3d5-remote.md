...
- **キュー上限 (critic D1・確定)**: `RemoteSyncQueue` はセッション内で**無制限に成長させない**。remote が長時間落ちて未送信が積み上がる場合に備え、**保持件数の上限 (例: 直近 N 件、初期値は実装時に設定)** を設ける。上限超過時は**古い未送信を落とさず、catch-up の全件 pull による回収に委ねる** (ローカル正典は daemon に完全保存されているので、remote に無い分は catch-up で再構成できる)。→ キューはあくまで「push 待ちの近況バッファ」で、真の source of truth はローカル正典。上限で捨ててもデータは失われない。上限値と超過時の UI 表現 (「未同期 N 件以上」等) は W3d5-3/-6 で確定。

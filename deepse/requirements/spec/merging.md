...
implicit merge が作った fork は, 後から見ると**なぜ存在するのかが分からない**. explicit merge の DtR graph は競合を引き起こした merge 操作に紐づくが, fork には紐づく操作がない. implicit merge そのものを書かないと決めた以上, 「どの競合がこの fork を生んだか」を指す先が op-log のどこにもないからである.

そこで **fork には, それを生んだ競合の記述を付ける.**

### 再計算ではなく, 検出時点で凍結する

「implicit merge は冪等な導出なのだから, 競合も畳み直せば再現できる」は成り立たない.

- 畳み直して得られるのは **今**の競合であって, fork を作った時点の競合ではない. fork の後も両側の op-log は伸びる
- そもそも競合が**消えていることがある**. 負けた側の要素がその後に削除されたり, 値が上書きされたりすれば, 畳み直しても競合は出てこない. その時 fork は**理由のない fork** になる

これは fork を書くと決めた理由 (「同期のたびに再計算すると, 解決したはずの fork が毎回復活する」) の裏返しである. 再計算に委ねると, 今度は**説明の方が消える**.

したがって競合の記述は, **検出した時点の内容を凍結して, fork とともに書く**.

### 何を書くか

人間が後から (あるいは fork の通知を受けた時に) 「何でこれが生じたんだ?」に答えられるだけのものを, **生きた問い合わせなしで**書く.

| | 内容 | なぜ要るか |
| --- | --- | --- |
| 種別 | content / structure (削除依存か並行変更か) / layout | 3 段のどれとして扱われたか |
| 対象 | 要素の id と, **その時点で人間に見える形** (node の内容の冒頭, プロパティ名など) | id だけでは, 後でその要素が消えていると何も分からない |
| 双方の操作 | 対立した二つの op と, それぞれの actor と clock | 「誰と誰が, いつ」が分からなければ, 相談する相手も選べない |
| 分岐点 | どこまで畳んだ状態で検出したか | 「その時点では何が見えていたか」を再現するため |

**これは記述であって, 何かの入力ではない.** 畳み込みはこの記述を読まないし, 何の分岐条件にもならない. 人間が読むためだけのものなので, 他の actor が書いた記述と一致している必要もない. だからこそ安く済む — システム的な機構ではなく, 記述で足りる.

ただし一つだけ用途がある. **この fork から DtR graph を起動するときの, 呼び出し対象の既定値を供給する** (→ [dialogueToResolveGraph](./dialogueToResolveGraph.md)). 既定値の供給源であって呼び出し対象そのものではない点は, 名簿と同じである.

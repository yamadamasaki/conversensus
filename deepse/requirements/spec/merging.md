...
**削除は推移的に及ぶので, 判定も推移的に行う.** node の削除は子孫の node と, 端点を失う edge を
カスケードして消す. したがって「削除された要素」は op に名前が書かれている要素だけではない.

| | trunk 側 | branch 側 | 何が起きるか |
| --- | --- | --- | --- |
| S1' | グループ G を削除 (G は node N を含む) | node N に edge を張る | **N は G の削除に巻き込まれて消えている**ので S1 と同じ結果になる |
| S2' | グループ G を削除 (G は node N を含む) | node N の内容を編集 | 同上, S2 と同じ結果 |

判定に必要なのは「その op に書かれた id」ではなく **「分岐点の状態にカスケードを当てて求めた, 実際に
消える要素の集合」**である. したがって競合の判定は分岐点のグラフを入力に取る必要がある.

直接参照だけを見ると, **グループ単位の削除と, その中身への参照の衝突を取り逃す**. グループは
まとめて消す操作なので, 共同作業では直接参照より起こりやすい.

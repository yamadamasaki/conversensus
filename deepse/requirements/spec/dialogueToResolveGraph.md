...
- (3) の場合には, この競合によって作られた fork に
  ...
  - **fork には, それを生んだ競合の記述が凍結して付いている** (→ [merging](./merging.md)「fork に競合の原因を記述する」). explicit merge と違って fork には紐づく merge 操作がないので, これが「何がこの DtR を必要にしたか」を指す唯一の手掛かりである
...
- この解決に参加してほしい actor の呼び出し (通知自体は step 2 では他の手段で行う. → step 3)
  - デフォルトの呼び出し対象の actor は
    ...
    - implicit merge の場合は, 自分だけ
      ...
      - あるいは競合している操作を行った actor たち. **これは fork に凍結された競合の記述から引く** — 畳み直して求めるのではない (その時には競合が消えていることがある)
    - 起動された resolve graph を見て, 対象を追加/削除できる
  - **呼び出し対象は起動時に確定し, 記録される.** 名簿は既定値を供給するだけであって, 呼び出し対象そのものではない (後述の「承認の判定」)
  - **誰を呼ぶかの判断材料として, その競合に関わる変更を行った actor を表示する.** 競合の各辺は, それを生んだ操作がどの batch にあるかを持っており, batch は actor を持つので, op-log を遡れば導ける. 「揉めている当人が誰か」が分からないまま呼ぶ相手を選ぶのは難しい
    - なお「誰がこの node/edge を**最後に**変更したか」を装飾として示す案 (後述) とは別物である. こちらは呼び出し対象の選択に直接使う
...
名簿は共同作業者の間で食い違ってよいと決めた (→ [participation](./participation.md)「名簿の食い違い」). したがって「全員が承認したか」を**名簿への生きた問い合わせ**として書くと, a の手元では「全員承認済 → 再 merge 可能」, b の手元では「1 人足りない → 保留」が**同時に, どちらも正常な状態として成立してしまう**. 再 merge は trunk を書き換えるので, これは表示の食い違いでは済まない.

そこで次のように定める.

- **呼び出し対象は DtR の起動時に確定し, 記録する.** 名簿はその既定値を供給するだけである. 起動した actor の名簿が古くても, 記録された集合は全員の手元で一致する
- **再 merge は pre 条件つきの操作である.** pre 条件は「記録された呼び出し対象の全員の承認が, この操作より前に記録されていること」であり, 満たさない再 merge は畳み込みの段階で捨てられる
  - これは名簿の op と同じ仕組みである (→ [participation](./participation.md)「pre 条件の検証」)
  - 古い情報を見て早まった再 merge を出しても, **それを出した本人の手元でも捨てられる**. 端末によって結論が変わることがない
- **呼び出された actor が承認しないまま参加を取りやめたら, 呼び出し対象から自動的に外れる.** 残りの全員で判定する. そうしないと, 去った人の承認を待って DtR が永久に決着しなくなる. 参加の取りやめも記録されるので, この判定も全員の手元で一致する

**要点は「全員」を可変な問い合わせではなく, 記録された集合に対する判定にすることである.** 承認は積み上がる一方なので, 集合を固定すれば判定は単調になり, 不完全な情報しか持たない手元でも早まった結論を出せない.

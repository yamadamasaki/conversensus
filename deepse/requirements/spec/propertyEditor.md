...
**step 2 では型を値から推論して表示するだけ**とし, 保存も検証も行わない. `node.setProperty` / `edge.setProperty` は `{名前, 値}` であって型を持たないので, 型を保存するには op の語彙を変える必要があるが, チェックしない型を保存する意味がない.

したがって **step 2 には「型が先に決まっている」という状態が存在しない** (型は値の従属変数である). 次の段落はその状態が生まれる step 3 の記述である.

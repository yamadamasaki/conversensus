...
| D7 | 同期対象の範囲 | **structure / content に加え layout (位置・大きさ) も同期対象**とする。presentation (色・線種) のみローカル限定 | layout は形式的 LPG 外だが、配置が変われば人間が読み取る意味も変わるためユーザにとって本質的 (特に `group` ノードは位置・大きさ自体が意味)。presentation はルールから決定論的に導出でき各端末で再導出可能なので共有不要 (§4) |
...
- **同期対象は structure / content / layout。presentation (色・線種など) のみ per-user・ローカル限定**とする。presentation はユーザが直接指定するものではなく、システムがルールに基づき決定論的に導出するため、同期せずとも各端末で再導出でき、共有する必要がない。
- **layout (位置・大きさ) は同期対象に含める。** layout は LPG の形式的定義には含まれないが、同じ nodes/edges でも配置が異なれば人間は異なる意味を読み取るため、ユーザにとって本質的な情報である。特に `group` ノードは位置・大きさそのものが意味を持つ。ただし layout の並行変更は **LWW で静かに解決**し、content のような「対立」可視化は行わない (配置の相違は合意形成の対象ではないため)。これによりコンフリクトの可視化対象は content 変更に絞られる (atproto-integration-strategy の方針を踏襲)。
...
| `category` | structure・content・layout=同期 / presentation=ローカル限定 の振り分け |

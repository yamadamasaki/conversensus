...
>
> 記号の凡例 (R = 要修正 / D = 要検討 / Q = 改善 / T = テスト / N = 対応中に見付かったもの):
...
| T2 | 貼り付け振り分けのテスト | テスト | **対応済** (`images/pasteImage.ts` へ切り出し) |
| T3 | 旧データを入力にしたテスト | テスト | **対応済** (移行の規則と `commitUrl` 経路) |
| — | Cmd+V の二重発火 | 未検証 | **対応済** (keydown 側が譲る。実機確認は残) |

**2026-08-13 時点でレビューの指摘は全件対応済**である。残るのは §9 N1
(死んだ record 型の削除) — 本 PR の範囲外として別課題にする。

#### T2 の対応 (2026-08-13)

§9 N3 の通り両端 (`pasteTarget` / `replaceNodeImage`) はテスト済で, 残るのは
振り分けの分岐だけだった。**`GraphEditor` を丸ごと描く代わりに, 振り分けを
`images/pasteImage.ts` へ出した** — Q1 で手続きを出したのと同じ形である。
依存 (`pickTarget` / `addImageNode` / `replaceImage`) を差し込み,
どちらが呼ばれたかを観測する (`images/pasteImage.test.ts`, 6 件)。
`GraphEditor` に残るのは位置決めとクリップボード API の吸収という配線だけ。

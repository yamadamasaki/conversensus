...
### readBranchSheets (p5-4 で追加)

hook は「現在 / 分岐点 / 直近コミット時点」の 3 つを同時に要る (画面 / diff ハイライト /
未コミット変更の判定)。旧経路はこれらを PDS スナップショットの控えとして React state に
抱えていたが、**op-log ではすべて同じログの切り出し**なので保持しない。1 回の読取から
3 つを導出することで、控えの持ち方に起因する食い違い自体を無くす。

 ...
 * その File を読み取り専用にする。
 *
 * ## ⚠️ 読み取り専用の窓は人には見えない (2026-09-06 実機)
 *
 * 義務が立つのは**再参加した File を開いた直後**で、開いた契機のサイクルがすぐ走るので
 * **バナーが人の目に留まる時間が無い**。実機で確かめて「見えない」ことを確認した。
 *
 * **それでよい、という判断である。**この機構が守っているのは人ではなく**相手の
 * projection** で、押さえたいのは「追いつく前に書けてしまう窓」そのものだからである。
 * 見せるための機構ではない。
 *
 * 逆に言えば、**この窓が人に見えるほど長くなったときは同期が遅すぎる**ということでも
 * ある。バナーはそのときに効く。
 *
 * 「同期のズレをどう扱うか」(遅い PDS・大きな File で窓が伸びる場合、待たせるのか
 * 別の見せ方をするのか) は**まだ決めていない**。
 *

...
template が当たっているノードは、本文の上に**種別名**を出す。`content` (markdown の本文) と
`label` (種別名) は別物で、P0 で語を分けた 2 つがここで初めて**同時に画面に出る**。

- **種別があれば本文と並べて出す / 無ければ何も出さない**: template が当たっていない
  普通のグラフに、意味の種別の場所を空けてはいけない (設計 D3)。
- **種別に編集の口を出さない**: toulmin node の種別とラベルは**作成時に決まり、その後
  変更できない** (仕様 OnMutation)。`button` でも `input` でもないことを見るのは、
  「押しても編集に入らない」を構造で固定するためである。本文が `onDoubleClick` で
  編集に入るので、隣に並ぶ要素が同じ振舞いをしないことは明示しておく価値がある。

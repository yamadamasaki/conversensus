...
## ⚠️ 別の窓が同じデーモンに書いた分 (#202, 2026-09-06 実機で発覚)

**判定は「このブラウザが追記したか」ではなく「手元の正典が動いたか」である。**

2 つのブラウザが同じ `localhost:5173` を開くと、**デーモンは 1 つ**なのでローカル正典は
共有されている。相手の窓の編集はもう正典に入っており、こちらの受信サイクルでは
`appendReceivedBatches` が 0 を返す。`onReceived` は `appended > 0` のときだけ鳴るので、
**PDS からは取れているのに画面を差し替える契機が鳴らない**。古いのは正典ではなく画面である。

- **別の窓が同じデーモンに書いた分は `appended=0` でも画面に出る。**着地を 0 件に固定した
  まま正典だけを動かし、`syncNow` の後に `activeFile` と `receiveEpoch` を見る。
  この 2 つを分けないと「たまたま受信で入った」と区別がつかない
- **手元の正典が動いていなければ swap も epoch 増加も起きない。**上の裏返しで、
  何も動いていなければ何回サイクルが回っても画面は差し替わらない

物差しは**他 actor の batch の数**である。自分の編集では増えないので、自分で描いている間に
不要な差し替えが走らない (走ると `receiveEpoch` が進んで React Flow が seed し直し、
選択が落ちる)。op-log は追記しかされないので単調に増え、増えていれば必ず画面に無いものが
正典にある。

契機が 2 つ (受信の着地 / 正典が動いた) になったので、**同じサイクルで 2 回差し替えない
柵**を置いた。2 回走らせても結果は同じだが `receiveEpoch` が 2 つ進む。

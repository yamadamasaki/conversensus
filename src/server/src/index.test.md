...
`POST /blobs` / `GET /blobs/:cid` — 画像などのバイナリの格納と取得。

### なぜ

この境界が **cid の計算とサイズ上限の enforcement を担う**。

- **cid はサーバが計算する**。クライアントの申告を鍵に使うと、内容と一致しない cid で
  ストアを汚染できてしまう。クライアントも同じ値を `computeBlobCid` で先に計算できるので、
  値を往復させる必要は無い
...
- `POST /blobs` が `{cid, mimeType, size}` を 201 で返す。cid は実機 PDS が返す値と同じ
  (`hello` のベクタを shared/blob.test.ts と共有している)
- 同じ内容を 2 回送っても同じ cid が返る (冪等)

...
### sheet.create の templateIds (Phase 5 P3)

シートに当てる template を **`sheet.create` が作成時に持つ** (設計 D1)。後から変える op は
作らない — 外したときに既存ノードの種別をどう扱うかを、答える材料の無いうちに決めることに
なるためである (step3)。

固定するのは以下である。

- **projection から `Sheet` まで運ぶ**: `FileStructure` の中で止まらず、UI が読む形まで
  届くこと。種別メニューを出すか出さないかの判断はこの値だけを見る。
- **`templateIds` を持たない既存の op-log はそのまま通る**: フィールドが optional なので
  **移行が要らない**ことを固定する。op を足すのにマイグレーションが要らないのは Phase 3 で
  確かめた性質だが、外に出る型 (`Sheet`) が変わるのはここが初めてなので改めて見る。
- **`sheet.setName` では紐づけが変わらない**: 紐づけを持つのは `sheet.create` だけである。
  他の sheet op がメタを上書きする実装だと、改名しただけで template が外れる。
- **消して作り直すと紐づけも作り直される**: `sheet.create` は add-wins (再作成が tombstone を
  解除する) なので、**再作成の op に載っている値が新しい紐づけになる**。古い紐づけが
...
後から `node.setLabel` で変えられるのだから作成時は要らない、とはならない。
**作成時に決まっているものを別 op に割ると「1 ユーザー操作 = 1 batch」が崩れ、
undo が 2 段になる** — 種別を選んで作ったノードを 1 回 undo すると、種別だけ消えて
ノードが残る。

- **作成時の種別をそのまま持つ / 種別なしで作れば持たない**: 省略が「種別なし」であること。
- **後から `node.setLabel` で上書きできる**: 2 つの経路が同じフィールドに落ちること。
- **空文字を載せると種別が外れる**: 外すことと「最初から無い」ことは、値としては
  空文字と `undefined` で分かれる。projection は空文字を保つ (op に載った事実を消さない)。

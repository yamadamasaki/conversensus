...
### 意味論の op はシートのスコープに属さない (Phase 6 L0)

`branch.*` / `commit.add` は「どのシートの話か」を持たない。判定は冒頭の `isFileOp` による
早期 continue **一本**に依存していて、そこを外すと sheetId のない branch batch が丸ごと
`no-scope` で落ちたことになる。

**なぜテストで見るしかないか** — `isFileOp` は器と意味論の**和**なので、和の片方を落としても
型は通る (`op is FileOp` の述語は実装の健全性を検査しない)。実際、層を分けた直後に
`isFileOp` を器だけに変える変異を入れたところ、**1779 件が全部緑のままだった**。
層を作った当の目的が、どのテストにも固定されていなかったということである。

**なぜ projection 側ではなくここで固定するか** — `projectBatches` に同じ変異を入れても
観測できない。`applyOp` の switch が意味論の op にあたる case を持たないので、素通りしても
結果が変わらないからである。振る舞いの差が出るのは適用可能性の判定だけなので、
層の契約はここに置く。

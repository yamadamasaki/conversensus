...
同期する branch を手元で物理的に消しても、畳み直せば復活し、相手からは最初から消えない。
そこで削除を `branch.remove` op にし、**一度消したら戻らない** (`file.remove` と同じ
remove-wins) とした。

- **branch もそのコミットも見えなくなる**。
- **後から同じ id を作っても出てこない**: 削除は sticky である。
- **fork の別名を消すと正の fork ごと消え、同じ競合の fork は復活しない**: 誰が書いた
  fork の id で消しても、畳んだ 1 つの fork が消える。
- **削除が作成より前に並ぶ log でも同じ結果**: 因果に反するが、参加期間のフィルタなどで
  起こりうる。**削除は畳み込みの最後にまとめて、別名を解決してから当てる。**見つけた時点で
  当てると、正の fork が作られる前に消えてしまい、同じ競合の fork が別の id で復活する。
...
- **削除された id は結果に現れない**: remove-wins の一般命題。

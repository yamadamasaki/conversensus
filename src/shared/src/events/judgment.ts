...
export const JudgmentOpSchema = z.discriminatedUnion('kind', [
   ...
   *
   * **グラフの op-log には触らない。**書くのは判断 op 1 つだけである。ローカル正典は
   * その actor の権威なので、引き取った本人の手元は**今の状態のまま**で、共有を止めた
   * 後にローカルで加えた変更も残る。一方、引き取った後に呼ばれた人が受け取るのは
   * **著者の参加期間の中の op だけ**なので (`participationFilter`)、空白期間の変更は
   * 届かない — **両者の projection は食い違う**。意図した動作である
   * (`deepse/requirements/spec/participation.md`「引き取りは『どの状態』から始まるか」)。
   *

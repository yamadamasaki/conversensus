...
- **座標の整数化 (ANA-125 S4)**: 小数の x / y / width / height を持つ snapshot から genesis したとき, `node.setLayout` op の値が整数になっていることを確認する。**ATProto のデータモデル (DAG-CBOR) には float 型が無く**, 小数を含む op を載せた batch は PDS の `putRecord` が 400 で弾く。しかも outbox は先頭が詰まると以降が全部止まるので, **1 つの小数が同期を丸ごと殺す**。編集経路 (`client/events/toUnified.ts`) では W3d5-7 で対処済みだったが **genesis 側の生成口が素通しのまま残っており**, 実 Safari の使い込みで「import した後どのファイルの操作でも `got 740.5` が出続ける」形で表面化した。生成口を shared の `nodeSetLayoutOp` に一本化した上で, ここでは**値が整数であること**を固定する (型では防げないため)。
